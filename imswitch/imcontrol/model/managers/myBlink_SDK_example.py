...
if num_boards_found.value == 1:
    ...
    if width.value == 1024:
        print("in this way the if condition is called. Below structure does not call the LUT Files???????")
    if width == 512:
        if depth == 8:
    ...
    retVal = slm_lib.Write_image(board_number, ImageOne.ctypes.data_as(POINTER(c_ubyte)), height.value*width.value*Bytes.value, wait_For_Trigger, flip_immediate, OutputPulseImageFlip, OutputPulseImageRefresh, timeout_ms)
    if(retVal == -1):
        print ("DMA Failed")
        slm_lib.Delete_SDK()
    else:
        ...
        for i in range(0, 5):
            ...
            retVal = slm_lib.Write_image(board_number, ImageTwo.ctypes.data_as(POINTER(c_ubyte)), height.value*width.value*Bytes.value, wait_For_Trigger, flip_immediate, OutputPulseImageFlip, OutputPulseImageRefresh,timeout_ms)
            if(retVal == -1):
                print ("DMA Failed")
                break
            else:
