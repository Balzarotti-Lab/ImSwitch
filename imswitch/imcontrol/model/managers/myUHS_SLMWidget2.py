...
# library
cdll.LoadLibrary("C:\\Users\\simon.roschger\\Desktop\\SDK\\Blink_C_wrapper")
...
cdll.LoadLibrary("C:\\Users\\simon.roschger\\Desktop\\SDK\\ImageGen")
...
class App(QWidget):
    ...
    def init_SLMController(self):
        ...
        if num_boards_found.value == 1:                         # for testing without PCIe card
            print ("Blink SDK was successfully constructed")
            print ("Found %s SLM controller(s)" % num_boards_found.value)

            self.height_ = c_uint(self.slm_size)                # for testing without PCIe card     
            self.width_ = c_uint(self.slm_size)
            self.depth_ = c_uint(8)
            ...
            self.blankImg = numpy.zeros([self.width_.value*self.height_.value*self.bytes.value], numpy.uint8, 'C')
            ...
            retVal = slm_lib.Write_image(board_number, self.blankImg.ctypes.data_as(POINTER(c_ubyte)), self.height_.value*self.width_.value*self.bytes.value, wait_For_Trigger, flip_immediate, OutputPulseImageFlip, OutputPulseImageRefresh, timeout_ms)
            if (retVal == -1):
    ...
    def openFileNameDialog(self):
        options = QFileDialog.Options()            # ?? could be removed
        # options |= QFileDialog.DontUseNativeDialog
        img, _ = QFileDialog.getOpenFileName(self,"Select existing mask file", "",'Images (*.png *.bmp)', options=options)
        if img:
            image = QImage(img)
            ...
            # iamge.save('C:\\Users\\simon.roschger\\Desktop\\SLM_repo\\testsave.bmp', 'bmp')
            # img2 = Image.fromarray(self.upload_img.reshape(1024,1024), 'L')                                             # reversing 1d array to image 
            # img2.save('C:\\Users\\simon.roschger\\Desktop\\SLM_repo\\reversed.bmp', 'bmp')
            # img3 = QImage('C:\\Users\\simon.roschger\\Desktop\\SLM_repo\\reversed.bmp')
            # image2 = img3.convertToFormat(QImage.Format_Grayscale8)
            # data2 = numpy.array(image2.bits().asarray(self.slm_size * self.slm_size))
            # print(data2)
            # img2.save('C:\\Users\\simon.roschger\\Desktop\\SLM_repo\\reversed.bmp', 'bmp')
    ...
    def uploadMask(self):
        ...
        retVal = 1                                                                                  # remove     
        # retVal = slm_lib.Write_image(board_number, self.upload_img.ctypes.data_as(POINTER(c_ubyte)), self.height_.value*self.width_.value*self.bytes.value, wait_For_Trigger, flip_immediate, OutputPulseImageFlip, OutputPulseImageRefresh, timeout_ms)
