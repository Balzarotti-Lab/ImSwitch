...
_logsDir = os.path.join(dirtools.UserFileDirs.Root, 'recordings',
                        'logs_etsted')
...
class EtSTEDController(ImConWidgetController):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__coordTransformHelper = EtSTEDCoordTransformHelper(
            self, self._widget.coordTransformWidget, _logsDir)
        ...
        self._widget.recordBinaryMaskButton.clicked.connect(
            self.initiateBinaryMask)
        self._widget.loadScanParametersButton.clicked.connect(
            self.getScanParameters)
        self._widget.setUpdatePeriodButton.clicked.connect(
        ...
        self._commChannel.sigSendScanParameters.connect(
            lambda analogParams, digitalParams, positionersScan: self.
            assignScanParameters(analogParams, digitalParams, positionersScan))
        self._commChannel.sigSendScanFreq.connect(
            lambda scanFreq: self.logScanFreq(scanFreq))
    ...
    def initiate(self):
        ...
        if not self.__running:
            ...
            scanInitiationType = self._widget.scanInitiation[
                scanInitiationTypeIdx]
            ...
            self.experimentMode = self._widget.experimentModes[
                experimentModeIdx]
            ...
            self._master.lasersManager.execOn(self.laserFast,
                                              lambda l: l.setEnabled(True))
    ...
    def scanEnded(self):
        ...
        if self.scanInitiationMode == ScanInitiationMode.ScanWidget:
            ...
            frame_period = self.scanInfoDict[
                'scan_samples_frame'] * 10e-6  # length (s) of total scan signal
    ...
    def loadTransform(self):
        ...
        self.transform = getattr(importlib.import_module(f'{transformname}'),
                                 f'{transformname}')
    ...
    def addImgBinStack(self, detectorName, img, init, scale,
        ...
        if detectorName == self.detectorFast:
            if self.__binary_stack is None:
                ...
                self._commChannel.sigUpdateImage.disconnect(
                    self.addImgBinStack)
                ...
                if np.ndim(self.__binary_stack) == 2:
                    ...
                    self.__binary_stack = np.concatenate(
                        (self.__binary_stack, [img]), axis=0)

    def calculateBinaryMask(self, img_stack):
        """ Calculate the binary mask of the region of interest. """
        img_mean = np.mean(img_stack, 0)
        img_bin = ndi.filters.gaussian_filter(
            img_mean, np.float(self._widget.bin_smooth_edit.text()))
        self.__binary_mask = np.array(
            img_bin > np.float(self._widget.bin_thresh_edit.text()))
    ...
    def setAnalysisHelpImg(self, img_ana, exinfo=None):
        ...
        self._widget.analysisHelpWidget.img.setImage(img_ana,
    ...
    def launchHelpWidget(self):
        ...
        self._widget.launchHelpWidget(self._widget.analysisHelpWidget,
                                      init=True)
    ...
    def runPipeline(self, detectorName, img, init, scale, isCurrentDetector):
        ...
        if detectorName == self.detectorFast:
            if not self.__busy:
                ...
                self.setDetLogLine("pipeline_start",
                                   datetime.now().strftime('%Ss%fus'))
                self.__busy = True
                t_pre = millis()
                if self.__runMode == RunMode.Visualize or self.__runMode == RunMode.Validate:
                    coords_detected, self.__exinfo, img_ana = self.pipeline(
                        img, self.__prevFrames, self.__binary_mask,
                        (self.__runMode == RunMode.Visualize
                         or self.__runMode == RunMode.Validate), self.__exinfo,
                        *self.__param_vals)
                else:
                    coords_detected, self.__exinfo = self.pipeline(
                        img, self.__prevFrames, self.__binary_mask,
                        self.__runMode == RunMode.Visualize, self.__exinfo,
                        *self.__param_vals)
                t_post = millis()
                self.setDetLogLine("pipeline_end",
                                   datetime.now().strftime('%Ss%fus'))
                    ...
                    if self.__runMode == RunMode.Visualize:
                        ...
                        if self.__validating:
                            if self.__validationFrames > 5:
                                self.saveValidationImages(prev=True,
                                                          prev_ana=True)
                            ...
                            self.setDetLogLine("fastscan_x_center",
                                               coords_scan[0])
                            self.setDetLogLine("fastscan_y_center",
                                               coords_scan[1])
                            # log all detected coordinates
                            if np.size(coords_detected) > 2:
                                for i in range(np.size(coords_detected, 0)):
                                    self.setDetLogLine("det_coord_x_",
                                                       coords_detected[i,
                                                                       0], i)
                                    self.setDetLogLine("det_coord_y_",
                                                       coords_detected[i,
                                                                       1], i)
                        ...
                        self.pauseFastModality()
                        self.setDetLogLine("coord_transf_start",
                                           datetime.now().strftime('%Ss%fus'))
                        coords_center_scan = self.transform(
                            coords_scan, self.__transformCoeffs)
                        self.setDetLogLine("slowscan_x_center",
                                           coords_center_scan[0])
                        self.setDetLogLine("slowscan_y_center",
                                           coords_center_scan[1])
                        self.setDetLogLine("scan_initiate",
                                           datetime.now().strftime('%Ss%fus'))
                        ...
                        self.updateScatter(np.flip(np.copy(coords_detected)),
                                           clear=True)
    ...
    def initiateSlowScan(self, position=[0.0, 0.0, 0.0]):
        ...
        print(
            f'Time for curve parameters: {time_curr_mid-time_curr_before} ms')
        print(
            f'Time for signal curve generation: {time_curr_after-time_curr_mid}'
        )
    ...
    def setCenterScanParameter(self, position):
        ...
        for index, positionerName in enumerate(
            ...
            if positionerName not in self._positionersScan:
                ...
                self._master.positionersManager[positionerName].setPosition(
                    position, 0)
                self.__logger.debug(
    ...
    def addFastAxisShift(self, center):
        ...
        C = np.array([
            -5.06873628, -80.6978355, 104.06976744, -7.12113356, 8.0065076,
            0.68227188
        ])  # second order plane fit
        params = np.array([
            px_size**2, dwell_time**2, px_size * dwell_time, px_size,
            dwell_time, 1
        ])  # for use with second order plane fit
    ...
    def setCentersScanWidget(self):
        devices = []
        centers = []
        for device, center in zip(self._analogParameterDict['target_device'],
                                  self._analogParameterDict['axis_centerpos']):
    ...
    def updateScatter(self, coords, clear=True):
        ...
        if np.size(coords) > 0:
            ...
            self._commChannel.sigAddItemToVb.emit(
                self._widget.getEventScatterPlot())
    ...
    def saveValidationImages(self, prev=True, prev_ana=True):
        ...
        if prev:
            ...
            self._commChannel.sigSnapImgPrev.emit(self.detectorFast, img,
                                                  'raw')
...
class EtSTEDCoordTransformHelper():
    ...
    def __init__(self, etSTEDController, coordTransformWidget, saveFolder,
                 *args, **kwargs):
        ...
        etSTEDController._widget.coordTransfCalibButton.clicked.connect(
            self.calibrationLaunch)
        self._widget.saveCalibButton.clicked.connect(self.calibrationFinish)
        self._widget.resetCoordsButton.clicked.connect(
            self.resetCalibrationCoords)
        self._widget.loadLoResButton.clicked.connect(
            lambda: self.loadCalibImage('lo'))
        self._widget.loadHiResButton.clicked.connect(
            lambda: self.loadCalibImage('hi'))
    ...
    def calibrationLaunch(self):
        """ Launch calibration. """
        self.etSTEDController._widget.launchHelpWidget(
            self.etSTEDController._widget.coordTransformWidget, init=True)
    ...
    def calibrationFinish(self):
        ...
        for pos_px in self.__hiResCoordsPx:
            pos = (np.around(
                pos_px[0] * self.__hiResPxSize - self.__hiResSize / 2,
                3), -1 * np.around(
                    pos_px[1] * self.__hiResPxSize - self.__hiResSize / 2, 3))
        ...
        filename = os.path.join(self.__saveFolder,
                                name) + '_transformCoeffs.txt'
        ...
        for i in range(0, len(self.__loResCoords)):
            pos = self.poly_thirdorder_transform(self.__transformCoeffs,
                                                 self.__loResCoords[i])
            pos_px = (np.around(
                (pos[0] + self.__hiResSize / 2) / self.__hiResPxSize, 0),
                      np.around((-1 * pos[1] + self.__hiResSize / 2) /
                                self.__hiResPxSize, 0))
    ...
    def coordinateTransformCalibrate(self):
        ...
        # fit
        res_lsq = least_squares(self.poly_thirdorder,
                                initguess,
                                args=(xdata, ydata),
                                method='lm')
    ...
    def poly_thirdorder(self, a, x, y):
        ...
        for i in range(0, len(x)):
            c1 = x[i, 0]
            c2 = x[i, 1]
            x_i1 = a[0] * c1**3 + a[1] * c2**3 + a[2] * c2 * c1**2 + a[
                3] * c1 * c2**2 + a[4] * c1**2 + a[5] * c2**2 + a[
                    6] * c1 * c2 + a[7] * c1 + a[8] * c2 + a[9]
            x_i2 = a[10] * c1**3 + a[11] * c2**3 + a[12] * c2 * c1**2 + a[
                13] * c1 * c2**2 + a[14] * c1**2 + a[15] * c2**2 + a[
                    16] * c1 * c2 + a[17] * c1 + a[18] * c2 + a[19]
    ...
    def poly_thirdorder_transform(self, a, x):
        ...
        c2 = x[1]
        x_i1 = a[0] * c1**3 + a[1] * c2**3 + a[2] * c2 * c1**2 + a[
            3] * c1 * c2**2 + a[4] * c1**2 + a[5] * c2**2 + a[6] * c1 * c2 + a[
                7] * c1 + a[8] * c2 + a[9]
        x_i2 = a[10] * c1**3 + a[11] * c2**3 + a[12] * c2 * c1**2 + a[
            13] * c1 * c2**2 + a[14] * c1**2 + a[15] * c2**2 + a[
                16] * c1 * c2 + a[17] * c1 + a[18] * c2 + a[19]
